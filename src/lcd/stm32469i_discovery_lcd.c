...
void BSP_LCD_LayerDefaultInit(uint16_t LayerIndex, uint32_t FB_Address)
{
  ...
  DrawProp[LayerIndex].FBStartAdress = FB_Address;
}
...
uint32_t BSP_LCD_ReadPixel(uint16_t Xpos, uint16_t Ypos)
{
  ...
  if(hltdc_eval.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_ARGB8888)
  {
    ...
    ret = *(__IO uint32_t*) (DrawProp[ActiveLayer].FBStartAdress + (4*(Ypos*BSP_LCD_GetXSize() + Xpos)));
  }
  else if(hltdc_eval.LayerCfg[ActiveLayer].PixelFormat == LTDC_PIXEL_FORMAT_RGB888)
  {
    /* Read data value from SDRAM memory */
    ret = (*(__IO uint32_t*) (DrawProp[ActiveLayer].FBStartAdress + (4*(Ypos*BSP_LCD_GetXSize() + Xpos))) & 0x00FFFFFF);
  }
  ...
  {
    /* Read data value from SDRAM memory */
    ret = *(__IO uint16_t*) (DrawProp[ActiveLayer].FBStartAdress + (2*(Ypos*BSP_LCD_GetXSize() + Xpos)));
  }
  else
  {
    /* Read data value from SDRAM memory */
    ret = *(__IO uint8_t*) (DrawProp[ActiveLayer].FBStartAdress + (2*(Ypos*BSP_LCD_GetXSize() + Xpos)));
  }
...
{
  /* Clear the LCD */
  LL_FillBuffer(ActiveLayer, (uint32_t *)(DrawProp[ActiveLayer].FBStartAdress), BSP_LCD_GetXSize(), BSP_LCD_GetYSize(), 0, Color);
}
...
void BSP_LCD_DisplayStringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text, Text_AlignModeTypdef Mode)
{
  ...
  case CENTER_MODE:
    {
      refcolumn = Xpos - xsize / 2;
      Ypos = Ypos - DrawProp[ActiveLayer].pFont->Height / 2;
      break;
    }
  case LEFT_MODE:
    {
...
void BSP_LCD_DrawHLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length)
{
  ...
  Xaddress = (DrawProp[ActiveLayer].FBStartAdress) + 4*(BSP_LCD_GetXSize()*Ypos + Xpos);
...
void BSP_LCD_DrawBitmap(uint32_t Xpos, uint32_t Ypos, uint8_t *pbmp)
{
  ...
  /* Set the address */
  Address = DrawProp[ActiveLayer].FBStartAdress + (((BSP_LCD_GetXSize()*Ypos) + Xpos)*(4));
...
void BSP_LCD_DrawPixel(uint16_t Xpos, uint16_t Ypos, uint32_t RGB_Code)
{
  ...
  *(__IO uint32_t*) (DrawProp[ActiveLayer].FBStartAdress + (4*(Ypos*BSP_LCD_GetXSize() + Xpos))) = RGB_Code;
}
...
void BSP_LCD_SetFBStartAdress(uint8_t* addr) {
  DrawProp[ActiveLayer].FBStartAdress = addr;
}

uint32_t BSP_LCD_getRainbowColor(int index) {
    uint32_t argb = 0xFF000000;
    uint8_t* r = (uint8_t*)(&argb)+2;
    uint8_t* g = (uint8_t*)(&argb)+1;
